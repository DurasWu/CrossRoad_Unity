...
namespace NDIDataChannelReadLocal
{
	...
	static const FName GetNDCSpawnDataName(TEXT("GetNDCSpawnData"));
...
void UNiagaraDataInterfaceDataChannelRead::PostStageTick(FNDICpuPostStageContext& Context)
{
	...
	for (auto& EmitterInstanceDataPair : InstanceData->EmitterInstanceData)
	{
		...
		if (FNiagaraEmitterInstance* TargetEmitter = EmitterInstanceDataPair.Key)
		{
			...
			FNiagaraEmitterInstance* StatefulEmitter = TargetEmitter; //modify for 5.3
			{
...
#if USE_UE5_4_NDC
		else if (BindingInfo.Name == NDIDataChannelReadLocal::GetNDCSpawnDataName)
		{
			OutFunc = FVMExternalFunction::CreateLambda([this, FuncIndex](FVectorVMExternalFunctionContext& Context) { this->GetNDCSpawnData(Context); });
		}
#endif	
		else
		{
...
void UNiagaraDataInterfaceDataChannelRead::SpawnConditional(FVectorVMExternalFunctionContext& Context, int32 FuncIdx)
{
	...
	FNiagaraEmitterInstance* EmittterInst = SystemInstance->GetEmitterByID(EmitterID);
	...
	if(bSpawn)
	{
		...
		FNDIDataChannelRead_EmitterInstanceData& EmitterInstData = InstData->EmitterInstanceData.FindOrAdd(EmittterInst);
