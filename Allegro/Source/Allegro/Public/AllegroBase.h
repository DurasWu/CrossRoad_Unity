...
inline void AllegroVectorTranspose4x4(VectorRegister4f& A, VectorRegister4f& B, VectorRegister4f& C, VectorRegister4f& D)
{
...
inline void AllegroVectorTranspose4x4(VectorRegister4d& A, VectorRegister4d& B, VectorRegister4d& C, VectorRegister4d& D)
{
	//#TODO Cross Platform Transpose using VectorShuffle maybe ?
	AlignedDouble4 HA(A);
	AlignedDouble4 HB(B);
...
template<typename T> struct alignas(16) TMatrix3x4Transpose
{
	...
	TMatrix3x4Transpose(const TMatrix<T>& RESTRICT Mat)
	{
		...
		AllegroVectorTranspose4x4(A, B, C, D);
...
typedef UE::Math::TMatrix3x4Transpose<float> AllegroShaderMatrixT;	//this is the matrix we send to shader :|